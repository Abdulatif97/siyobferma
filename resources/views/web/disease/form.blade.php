...
<div class="box box-info padding-1">
    <div class="box-body">
        <div class="form-group">
            ...
            {{ Form::text('title', $disease->title ?? '', ['class' => 'form-control' . ($errors->has('title') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('locale.title')]) }}
            {!! $errors->first('title', '<div class="invalid-feedback">:message</p></div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label(trans('locale.description')) }}
            {{ Form::textarea('description', $disease->description ?? '', ['class' => 'form-control' . ($errors->has('description') ? ' is-invalid' : ''),   'rows' => 3, 'onkeypress' => "return nameFunction(event);", 'placeholder' => trans('locale.description')]) }}
            {!! $errors->first('description', '<div class="invalid-feedback">:message</p></div>') !!}
        </div>
      <fieldset class="form-group">
          {{ Form::label(trans('term.disease-category')) }}
          {{ Form::select('disease_category_id', $categories, $disease->disease_category_id ?? old('value') ,['class' => 'form-control' . ($errors->has('disease_category_id') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('locale.select')]) }}
          {!! $errors->first('disease_category_id', '<div class="invalid-feedback">:message</p></div>') !!}
        </fieldset>
      <fieldset class="form-group">
          {{ Form::label(trans('input.type')) }}
          {{ Form::select('type', $types, $disease->type ?? old('value') ,['class' => 'form-control' . ($errors->has('type') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('locale.select')]) }}
          {!! $errors->first('type', '<div class="invalid-feedback">:message</p></div>') !!}
        </fieldset>
      <fieldset class="form-group">
            {{ Form::label(trans('locale.deleted')) }}
            {{ Form::select('deleted', [1 => trans('locale.yes'), 0 => trans('locale.no')], $disease->deleted ?? 0 ,['class' => 'form-control' . ($errors->has('deleted') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('locale.select')]) }}
        ...
        <div class="product_content validate dropzone" id="products">
          <fieldset class="form-control2 " id="fieldID" data-required="false" >
            <div id="imguploaded" class="imgupload d-flex" style="height: 100%;min-height:200px; flex-wrap:wrap;" data-min-width="540">
              @if (isset($disease) && !empty($disease->attachments))
