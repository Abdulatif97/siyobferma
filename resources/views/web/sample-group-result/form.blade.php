...
<div class="box box-info padding-1">
    <div class="box-body">
      <div class="form-group">
        ...
        {{ Form::text('code', $sampleGroupResult->code?? '', ['class' => 'form-control' . ($errors->has('code') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('input.code')]) }}
        {!! $errors->first('code', '<div class="invalid-feedback">:message</p></div>') !!}
      </div>
      <fieldset class="form-group">
        {{ Form::label(trans('term.sample-group')) }}
        {{ Form::select('sample_group_id', $groups, $sampleGroupResult->sample_group_id ?? old('value') ,['class' => 'form-control' . ($errors->has('sample_group_id') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('locale.select')]) }}
        {!! $errors->first('sample_group_id', '<div class="invalid-feedback">:message</p></div>') !!}
      </fieldset>
      <div class="form-group">
        {{ Form::label(trans('input.temperature')) }}
        {{ Form::textarea('temperature', $sampleGroupResult->temperature ?? old('value'), ['class' => 'form-control' . ($errors->has('temperature') ? ' is-invalid' : ''),   'rows' => 3, 'onkeypress' => "return nameFunction(event);", 'placeholder' => trans('input.temperature')]) }}
        {!! $errors->first('temperature', '<div class="invalid-feedback">:message</p></div>') !!}
      </div>
      <div class="form-group">
        {{ Form::label(trans('input.hum')) }}
        {{ Form::textarea('hum', $sampleGroupResult->hum ?? old('value'), ['class' => 'form-control' . ($errors->has('hum') ? ' is-invalid' : ''),   'rows' => 3, 'onkeypress' => "return nameFunction(event);", 'placeholder' => trans('input.hum')]) }}
        {!! $errors->first('hum', '<div class="invalid-feedback">:message</p></div>') !!}
      </div>
      <div class="form-group">
        {{ Form::label(trans('input.additional_text')) }}
        {{ Form::textarea('additional_text', $sampleGroupResult->additional_text ?? old('value'), ['class' => 'form-control' . ($errors->has('additional_text') ? ' is-invalid' : ''),   'rows' => 3, 'onkeypress' => "return nameFunction(event);", 'placeholder' => trans('input.additional_text')]) }}
        {!! $errors->first('additional_text', '<div class="invalid-feedback">:message</p></div>') !!}
      </div>
      <div class="form-group">
        {{ Form::label(trans('input.end_date')) }}
        {{ Form::date('end_date', $sampleGroupResult->end_date ?? '', ['class' => 'form-control' . ($errors->has('end_date') ? ' is-invalid' : ''),'required' => 'required', 'placeholder' => trans('input.end_date')]) }}
        {!! $errors->first('end_date', '<div class="invalid-feedback">:message</p></div>') !!}
      </div>

      <div class="form-group">
        {{ Form::label(trans('input.approved_date')) }}
        {{ Form::date('approved_date', $sampleGroupResult->approved_date ?? '', ['class' => 'form-control' . ($errors->has('approved_date') ? ' is-invalid' : ''),'required' => 'required', 'placeholder' => trans('input.approved_date')]) }}
        {!! $errors->first('approved_date', '<div class="invalid-feedback">:message</p></div>') !!}
      </div>

      <div class="form-group">
            {{ Form::label(trans('input.note')) }}
            {{ Form::textarea('note', $sampleGroupResult->note?? '', ['class' => 'form-control' . ($errors->has('note') ? ' is-invalid' : ''),   'rows' => 3, 'onkeypress' => "return nameFunction(event);", 'placeholder' => trans('input.note')]) }}
            {!! $errors->first('note', '<div class="invalid-feedback">:message</p></div>') !!}
        </div>
      <fieldset class="form-group">
            {{ Form::label(trans('locale.deleted')) }}
            {{ Form::select('deleted', [1 => trans('locale.yes'), 0 => trans('locale.no')], $sampleGroupResult->deleted ?? 0 ,['class' => 'form-control' . ($errors->has('deleted') ? ' is-invalid' : ''), 'required' => 'required',  'placeholder' => trans('locale.select')]) }}
        ...
        <div class="product_content validate dropzone" id="products">
          <fieldset class="form-control2 " id="fieldID" data-required="false" >
            <div id="imguploaded" class="imgupload d-flex" style="height: 100%;min-height:200px; flex-wrap:wrap;" data-min-width="540">
              @if (isset($sampleGroupResult) && !empty($sampleGroupResult->attachments))
                @foreach ($disease->attachments as $key => $photo)
